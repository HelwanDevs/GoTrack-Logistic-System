...
export const SettingsPage = () => {
  ...
  const { data: currentUserData, error: currentUserError } = useAccountQuery(
    currentUser?.accountId ?? "",
  );

  const [editEmailForm, setEditEmailForm] = useState<EditEmailForm>({
    newEmail: currentUserData?.email || "",
    confirmEmail: currentUserData?.email || "",
  });
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
              ...
              <span className="inline-block px-3 py-1 rounded-full text-label-md font-label-md bg-primary text-on-primary">
                {getRoleLabel(currentUserData?.role || UserRole.EMPLOYEE)}
              </span>
            </div>
          </div>
        </div>
